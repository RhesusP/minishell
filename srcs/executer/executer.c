...
/*   Updated: 2023/06/22 16:22:51 by cbernot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_execve3(t_to_free to_free, t_env_var *path, int count, int nb_pipes)
{
	int		pid;
	int		status;
	t_redir	**redir;
...
void	execute_line(t_word	**word, t_env_var **env, t_env_var **global, char *line)
{
	...
	*(to_free.command) = 0;
	while (get_next_cmd(word, &to_free.command))
	{
		...
		if (!execute_non_fork_builtin(to_free, pipes_nbr))
			ft_execve2(to_free.command, path, to_free.tubes, count, pipes_nbr, to_free.env, to_free.global, to_free.lst);
		clear_word_lst(to_free.command);
		count++;
	}
	free_word_lst(to_free.command);
